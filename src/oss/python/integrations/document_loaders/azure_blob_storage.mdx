...
The `AzureBlobStorageLoader` loads all blobs from a given container in Azure Blob Storage. The loader returns [`Document`](https://reference.langchain.com/python/langchain_core/documents/?_gl=1*wrgkz*_gcl_au*MTY2OTY3Mzk1OS4xNzYwNDY2ODI3*_ga*NzYyOTQ1NTEyLjE3NTU4NzYzODk.*_ga_47WX3HKKY2*czE3NjExNTYwOTckbzUxJGcxJHQxNzYxMTU3NjU0JGo2MCRsMCRoMA..#langchain_core.documents.base.Document) objects containing the blob content (defaulting to UTF-8 encoding) and metadata including the blob URL, as shown in the example below.
...
```python
from langchain_azure_storage.document_loaders import AzureBlobStorageLoader
from langchain_community.document_loaders import PyPDFLoader  # This examples requires installing `langchain-community` and `pypdf`
